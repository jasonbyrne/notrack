...
function create_sqltables {
  ...
  # Live
  mysql --user=ntrk --password=ntrkpass -D ntrkdb -e "CREATE TABLE IF NOT EXISTS live (id BIGINT UNSIGNED NOT NULL PRIMARY KEY AUTO_INCREMENT, log_time DATETIME, sys TINYTEXT, dns_request TINYTEXT, dns_result CHAR(1));"
  # Historic
  mysql --user=ntrk --password=ntrkpass -D ntrkdb -e "CREATE TABLE IF NOT EXISTS historic (id BIGINT UNSIGNED NOT NULL PRIMARY KEY AUTO_INCREMENT, log_time DATETIME, sys TINYTEXT, dns_request TINYTEXT, dns_result CHAR(1));"
  # Users
  mysql --user=ntrk --password=ntrkpass -D ntrkdb -e "CREATE TABLE IF NOT EXISTS users (id INT UNSIGNED NOT NULL PRIMARY KEY AUTO_INCREMENT, user TINYTEXT, pass TEXT, level CHAR(1));"
  # Blocklist
  mysql --user=ntrk --password=ntrkpass -D ntrkdb -e "CREATE TABLE IF NOT EXISTS blocklist (id BIGINT UNSIGNED NOT NULL PRIMARY KEY AUTO_INCREMENT, bl_source VARCHAR(64), site TINYTEXT, site_reverse TINYTEXT, site_status BOOLEAN, comment TEXT);"
  ...
  # lightyaccess
  mysql --user=ntrk --password=ntrkpass -D ntrkdb -e "CREATE TABLE IF NOT EXISTS lightyaccess (id BIGINT UNSIGNED NOT NULL PRIMARY KEY AUTO_INCREMENT, log_time DATETIME, site TINYTEXT, http_method CHAR(4), uri_path TEXT, referrer TEXT, user_agent TEXT, remote_host TEXT);"
  # Config table
  mysql --user="$USER" --password="$PASSWORD" -D ntrkdb -e "CREATE TABLE IF NOT EXISTS config (config_id INT UNSIGNED NOT NULL PRIMARY KEY AUTO_INCREMENT, config_type VARCHAR(16), option_name VARCHAR(16), option_value TEXT, option_enabled BOOLEAN);"
